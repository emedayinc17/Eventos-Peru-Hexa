...
def test_create_servicio_admin(monkeypatch):
    app = FastAPI()
    # mock session
    monkeypatch.setattr(router_module, "get_db_session", lambda settings: iter([object()]))

    class MockRepo:
        def create_servicio(self, s, *, nombre, tipo_evento_id, descripcion=None):
            return {"id": "s-1", "nombre": nombre}

    monkeypatch.setattr(router_module, "get_create_servicio_use_case", lambda: type("X", (), {"execute": lambda self, session, **kw: MockRepo().create_servicio(None, **kw)})())

    app.include_router(router_module.create_router())
    client = TestClient(app)

    resp = client.post("/v1/admin/servicios", json={"nombre": "Iluminación", "tipo_evento_id": "t-1", "descripcion": "Luces"})
    assert resp.status_code == 201
    body = resp.json()
    assert body["id"] == "s-1"
    assert body["nombre"] == "Iluminación"
